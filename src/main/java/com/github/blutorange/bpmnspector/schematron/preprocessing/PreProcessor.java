...
/**
 * Does the preprocessing step for creating only one document contenting everything and a namespace table
 *
...
public class PreProcessor {
    ...
    /**
     * Performs the pre-processing step for creating one document including the content of all imported BPMN processes
     * of the document
     *
     * @param process the BPMNProcess which should be used as starting point
    ...
    public Document preProcess(BPMNProcess process) {
        ...
        List<Attribute> refAttributes =
                createXPathExpressionForAllQNameAttributes().evaluate(cloneOfDoc);
    ...
    /**
     * helper method to rename all global ids in the bpmn files to be able to merge them into one file and to detect
     * later the errors in the right files. the colon is replaced by an underscore because ids in the one merged file
     * can't contain colons
     *
     * @param process the document which should be validated
     * @param attribute the attribute with the ID to be renamed
     */
    private void renameGlobalIds(BPMNProcess process, Attribute attribute) {
        ...
        if (prefix.equals(JDOMUtils.getUsedPrefixForTargetNamespace(process.getProcessAsDoc())
                .orElse(""))) {
     ...
     *
     * @param process the document which should be validated
     * @param element the element with the ID to be renamed
     */
    ...
    /**
     * adds to all nodes new and unique prefixes in the given document for the validation process and violation
     * searching
     *
    ...
    private void renameIds(BPMNProcess importedProcess) {
        ...
        List<Attribute> attributes =
                createXPathExpressionForAllIDsAndReferenceAttributes().evaluate(importedProcess.getProcessAsDoc());
        List<Element> elements =
                createXPathExpressionForAllReferenceElements().evaluate(importedProcess.getProcessAsDoc());

        attributes.stream().filter(x -> !x.getValue().contains(":")).forEach(x -> {
            LOGGER.debug(
                    "Updating attribute ID '{}' new value: {}",
                    x.getValue(),
                    importedProcess.getGeneratedPrefix() + "_" + x.getValue());
            x.setValue(importedProcess.getGeneratedPrefix() + "_" + x.getValue());
        });
        elements.stream().filter(x -> !x.getText().contains(":")).forEach(x -> {
            LOGGER.debug(
                    "Updating element {} ID '{}' new value: {}",
                    x.getName(),
                    x.getText(),
                    importedProcess.getGeneratedPrefix() + "_" + x.getText());
            x.setText(importedProcess.getGeneratedPrefix() + "_" + x.getText());
        });
    }

    /**
     * adds the children of importDefinitionsNode to the definitionsNode of the given headFileDocument
     *
     * @param importedProcess the definitionsNode of the document, which should be added to the headFileDocument
     * @param headDocument the document, where the nodes should be added
     */
    ...
    private XPathExpression<Attribute> createXPathExpressionForAllQNameAttributes() {
        ...
        return xPathFactory.compile(ALL_QNAME_ATTRIBUTES, Filters.attribute(), null, ConstantHelper.BPMN_NAMESPACE);
    }
    ...
    private XPathExpression<Element> createXPathExpressionForAllQNameElements() {
        ...
        return xPathFactory.compile(ALL_QNAME_ELEMENTS, Filters.element(), null, ConstantHelper.BPMN_NAMESPACE);
    }

    /**
     * Creates an XPathExpression filtering all Attributes which define and use IDs (regardless of the used type (ID,
     * IDREF or QName))
     */
    private XPathExpression<Attribute> createXPathExpressionForAllIDsAndReferenceAttributes() {

        var expStr = "//bpmn:*/@id | " + ALL_IDREF_ATTRIBUTES + " | " + ALL_QNAME_ATTRIBUTES;

        return xPathFactory.compile(expStr, Filters.attribute(), null, ConstantHelper.BPMN_NAMESPACE);
    }

    /**
     * Creates an XPathExpression filtering all Elements which use IDs (regardless of the used type (ID, IDREF or
     * QName))
     */
    private XPathExpression<Element> createXPathExpressionForAllReferenceElements() {

        var expStr = ALL_IDREF_ELEMENTS + " | " + ALL_QNAME_ELEMENTS;

        return xPathFactory.compile(expStr, Filters.element(), null, ConstantHelper.BPMN_NAMESPACE);
    }
}
