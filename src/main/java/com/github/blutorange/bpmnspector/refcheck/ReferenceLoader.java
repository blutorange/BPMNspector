...
/**
 * This class loads the references form the references.xml file. The XML file needs to fit the references.xsd schema.
 * The references will be structured by the classes {@link BPMNElement} and {@link Reference}.
 *
...
public class ReferenceLoader {
    ...
    /**
     * This method loads the XML file and gives the data structure of BPMNElements and References back.
     *
     * @param referencesPath path to the XML file with the references
     * @param xsdPath path to the XSD file for the references
     * @return a hash map with the element names as keys and as value the BPMNElements which contain the references
     * @throws ValidationException if problems occurred while loading, traversing or checking the 'references.xml' file
     *     against its XSD
     */
    public Map<String, BPMNElement> load(String referencesPath, String xsdPath) throws ValidationException {
        try (var refPathStream = getClass().getResourceAsStream(referencesPath)) {
    ...
    private void createBPMNElementAndReferences(
            Element rootElem,
        ...
        if (!childElements.isEmpty()) {
            ...
            for (Element child : childElements) {
                ...
                // process child
                createBPMNElementAndReferencesForChild(
                        rootElem, child.getText(), elementName, processedElements, references);
            }
        }
    ...
    private void createBPMNElementAndReferencesForChild(
            Element rootElem,
            String nameOfChild,
            String nameOfParent,
            Map<String, BPMNElement> processedElements,
        ...
        if (childToProcess.isPresent()) {
            ...
            // no further references for child -> create BPMNElement for Child and add child directly with
            // referencesFromParent
    ...
    private void createReferencesOfElement(Element element, List<Reference> referenceList) {
        List<Element> referencesInFile = element.getChildren("reference");
        for (Element reference : referencesInFile) {
            ...
            boolean qname = convertToBoolean(reference.getAttributeValue("qname"));
            boolean attribute = convertToBoolean(reference.getAttributeValue("attribute"));
    ...
    private void validateReferencesFile(String referencesPath, String xsdPath) throws ValidationException, IOException {
        try (var refPathStream = getClass().getResourceAsStream(referencesPath);
                var xsdPathStream = getClass().getResourceAsStream(xsdPath)) {
            var schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
            var schema = schemaFactory.newSchema(new StreamSource(xsdPathStream));
            ...
            if (!xsdErrorList.isEmpty()) {
                var xsdErrorText = new StringBuilder(200)
