...
public class BPMNReferenceValidator implements BpmnProcessValidator {
    ...
    private static final org.slf4j.Logger LOGGER =
            LoggerFactory.getLogger(BPMNReferenceValidator.class.getSimpleName());

    /**
     * Constructor sets the defaults. And loads the reference definitions to be used for validations.
     *
     * @throws ValidationException if problems with the language files exist
     */
    ...
    /**
     * This method validates a set of BPMN processes given in the parameter process regarding reference correctness,
     * i.e., exist all references and are they referencing an allowed BPMN element.
     *
     * @param process the base process which should be checked; must not be <code>null</code>
     * @param validationResult the ValidationResult to be used; must not be <code>null</code>
    ...
    /**
     * This method loads the BPMN elements with the checkable references for the validation.
     *
     * @throws ValidationException if technical problems with the files "references.xml" and "references.xsd" occurred
     */
    private void loadReferences() throws ValidationException {
        ReferenceLoader referenceLoader = new ReferenceLoader();
        bpmnRefElements = referenceLoader.load(
                "/com/github/blutorange/bpmnspector/resources/references.xml",
                "/com/github/blutorange/bpmnspector/resources/references.xsd");
        var bpmnElementsLogText = new StringBuilder(500);
     ...
     *
     * @param baseProcess the ProcessFileSet of the file, which should be validated
    ...
    private void startValidation(BPMNProcess baseProcess, ValidationResultBuilder validationResult)
        ...
        JDOMUtils.getAllElementsGroupedByNamespace(importedElements, baseProcess, new ArrayList<>());

        // log ns prefixes of all imported files
        var importedFilesLogText =
                new StringBuilder(100).append("Elements found by namespace: ").append(System.lineSeparator());
        for (final var entry : importedElements.entrySet()) {
        ...
        while (list.hasNext()) {
            ...
            if (bpmnRefElements.containsKey(currentName)) {
                for (Reference checkingReference :
                        bpmnRefElements.get(currentName).getReferences()) {
                    ...
                    if (checkingReference.isAttribute()) {
                        ...
                        for (Element child : currentElement.getChildren()) {
                            if (child.getName().equals(checkingReference.getName())) {
                    ...
                    if (referencedId != null) {
                        LOGGER.debug("Checking the Reference: {} :: {}", currentName, checkingReference.getName());
                        referenceChecker.validateReferenceType(
                                elements,
                                importedElements,
                                validationResult,
                                currentElement,
