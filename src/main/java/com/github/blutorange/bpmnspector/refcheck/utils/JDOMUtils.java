...
public class JDOMUtils {
    ...
    /**
     * This method puts all elements with an id found in the given document into a hash map. The key is the id, the
     * value the element.
     *
    ...
    /**
     * Map which uses the namespace-URI as an ID and another Map as value. The inner HashMap contains all Elements
     * accessible via the ID as key {@link #getAllElements}
     *
     * @param elementsMap the Map to be populated
     * @param process the BPMNProcess to be added
     * @param alreadyVisited a List of filenames of already added files
     */
    public static void getAllElementsGroupedByNamespace(
            Map<String, Map<String, Element>> elementsMap, BPMNProcess process, List<String> alreadyVisited) {
        if (!alreadyVisited.contains(process.getBaseURI())) {
            addElementsFromSingleDocument(elementsMap, process.getProcessAsDoc());
    ...
    public static Optional<String> getUsedPrefixForTargetNamespace(Document document) {
        ...
        Optional<Namespace> namespaceForTargetNamespace = rootElem.getNamespacesInScope().stream()
                .filter(n -> n.getURI().equals(targetNamespaceURI))
                .findAny();
