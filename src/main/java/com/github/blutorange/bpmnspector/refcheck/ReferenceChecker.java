...
public class ReferenceChecker {
    ...
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(ReferenceChecker.class.getSimpleName());
    ...
    /**
     * This method validates the current element against the checking reference. Some already existing information will
     * be expected as parameters.
     *
     * @param elements the elements of the root file (= <code>bpmnFile</code>)
     * @param importedElements the elements which have been defined in imported files
     * @param validationResult the validation result for adding found violations
     * @param currentElement the current element to validate
     * @param line the line of the reference in the root file for the violation message
     * @param column the column of the reference
     * @param checkingReference the reference to validate against
     * @param referencedId the referenced ID
     * @param ownPrefix the namespace prefix of the current element
    ...
    public void validateReferenceType(
        ...
        if (checkingReference.isQname()) {
            ...
            if (referencedId.contains(":")) { // reference ID is prefixed and
                ...
                var parts = referencedId.split(":");
                var prefix = parts[0];
                var importedId = parts[1];
                ...
                if (ownPrefix.equals(prefix)) { // namespace is used for the root file
                    ...
                    if (elements.containsKey(importedId)) { // elem is in root file
                        checkTypeAndAddViolation(
                            ...
                            && relevantImportedElements.containsKey(
                                    importedId)) { // element is found in the imported file
                        checkTypeAndAddViolation(
                                validationResult,
                                line,
                                column,
            ...
            if (elements.containsKey(referencedId)) {
                checkTypeAndAddViolation(
                        validationResult, line, column, currentElement, checkingReference, elements.get(referencedId));
            } else {
                createAndAddExistenceViolation(validationResult, line, column, currentElement, checkingReference);
            }
        }
    ...
    /**
     * This method validates the type of the referenced element against the checking reference and adds a violation if
     * found.
     *
     * @param validationResult the violation list for adding found violations
     * @param line the line of the reference in the root file for the violation message
     * @param column the column of the reference
     * @param currentElement the current element to validate
     * @param checkingReference the reference to validate against
     * @param referencedElement the referenced element to validate
     * @throws ValidationException thrown if errors occur during creation of the Violation
     */
        ...
        if (referencedTypes != null) {
            ...
            if (!validType) {
                createAndAddReferenceTypeViolation(
                        validationResult, line, column, currentElement, checkingReference, referencedElement, types);
            }
        }
    }

    /**
     * Creates and adds a found reference type violation to the list of violations.
     *
     * @param validationResult the validationResult for adding the found violation
     * @param line the line of the reference in the root file
     * @param column the column of the reference
     * @param currentElement the element causing the violation
     * @param checkingReference the violated reference
     * @param referencedElement the actually referenced element
     * @param types a list of allowed reference types
     ...
     *
     * @param validationResult the validation result for adding the found violation
     * @param line the line of the reference in the root file
     * @param column the column of the reference in the root file
     * @param currentElement the element causing the violation
     * @param checkingReference the violated reference
     * @throws ValidationException thrown if the Validation could not be created
     */
    public void createAndAddExistenceViolation(
            ...
            Reference checkingReference)
            throws ValidationException {
        String message = ViolationMessageCreator.createExistenceViolationMessage(
                currentElement.getName(),
                checkingReference.getName(),
                line,
                ViolationMessageCreator.DEFAULT_MSG,
                XPathHelper.getAbsolutePath(currentElement));
        Violation violation =
                new Violation(createLocation(line, column, currentElement), message, CONSTRAINT_REF_EXISTENCE);
        validationResult.addViolation(violation);
    }
    ...
    private Location createLocation(int line, int column, Element currentElement) throws ValidationException {
        try {
            ...
            var locationPath = Paths.get(baseUri);
            return new Location(locationPath, new LocationCoordinate(line, column));
        } catch (URISyntaxException e) {
            throw new ValidationException(
                    "Base URI of current element " + currentElement.getName() + " could not be restored.", e);
        }
    }
}
