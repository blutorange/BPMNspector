...
public class FileUtils {
    ...
    public static Path createResourcesForReports() throws IOException {
        var reportDir = Paths.get("reports");

        if (!Files.exists(reportDir)) {
            Files.createDirectory(reportDir);
        }
        var reportResDir = reportDir.resolve("res");
        if (!Files.exists(reportResDir)) {
            Files.createDirectory(reportResDir);
        }
        Path myPath = null;
        var loadedFromJar = false;
        try {
            var uri = FileUtils.class.getResource("/reporting/res").toURI();
            ...
            Files.walk(myPath).forEach(path -> {
                try {
                    Files.copy(
                            path,
                            reportResDir.resolve(path.getFileName().toString()),
    ...
    public static Path createFileForReport(Path reportDir, String filePrefix, String fileSuffix) {
        var reportFile = reportDir.resolve(filePrefix + "_validation_result." + fileSuffix);

        for (var i = 1; Files.exists(reportFile); i++) {
