...
public class BpmnXPathHelper {
    ...
    private List<Element> findAllElementsByNameList(Element baseElem, List<String> names) {
        return baseElem.getChildren().stream()
    ...
    private int determineIndexForElementInsertion(Element parent) {
        ...
        List<String> allowedElementsBefore = Arrays.asList(
                "incoming", "categoryValueRef", "monitoring", "auditing", "extensionElements", "documentation");

        List<Element> children = parent.getChildren();

        for (String str : allowedElementsBefore) {
            List<Element> relevantElements =
                    children.stream().filter(c -> c.getName().equals(str)).collect(Collectors.toList());
