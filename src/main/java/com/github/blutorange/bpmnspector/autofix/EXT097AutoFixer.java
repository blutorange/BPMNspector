...
public class EXT097AutoFixer implements ViolationFixer {
    ...
    public boolean fixSingleViolation(Document processAsDoc, String xPath) {
        ...
        List<Element> childrenWithNeededIncomingSeqFlow =
                bpmnXPathHelper.determineFlowNodesWithNeededIncomingFlow(parentProcessElement);
        List<Element> unconnectedElems = childrenWithNeededIncomingSeqFlow.stream()
                .filter(e ->
                        e.getChild("incoming", Namespace.getNamespace(ConstantHelper.BPMN_NAMESPACE_STRING)) == null)
