...
public class EXT012SecondOptionMarkFormalExpFixer implements ViolationFixer {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(EXT012SecondOptionMarkFormalExpFixer.class.getSimpleName());
    ...
    private void addAttributeToChild(Element elementToUse, String nameOfChild) {
        Element childToUse =
                elementToUse.getChild(nameOfChild, Namespace.getNamespace(ConstantHelper.BPMN_NAMESPACE_STRING));
        if (childToUse != null) {
            childToUse.setAttribute(
                    "type", childToUse.getNamespacePrefix() + "tFormalExpression", getXSINamespace(childToUse));
        }
    }
