...
public class EXT105AutoFixer implements ViolationFixer {
    ...
    public boolean fixSingleViolation(Document processAsDoc, String xPath) {
        ...
        List<Element> childrenWithNeededOutgoingSeqFlow =
                bpmnXPathHelper.determineFlowNodesWithNeededOutgoingFlow(parentProcessElement);
        List<Element> unconnectedElems = childrenWithNeededOutgoingSeqFlow.stream()
                .filter(e ->
                        e.getChild("outgoing", Namespace.getNamespace(ConstantHelper.BPMN_NAMESPACE_STRING)) == null)
